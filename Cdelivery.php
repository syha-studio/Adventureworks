...
// Summary
    $item = query("SELECT COUNT('id') a FROM shipping_method");
// Chart Order and Income
    $datacat2 = mysqli_query($conn,"SELECT DISTINCT name FROM shipping_method");
    while($row = mysqli_fetch_array($datacat2)){
        ...
        $query = mysqli_query($conn,"SELECT SUM(LineTotal) AS 'Total Incomes', SUM(OrderQty) AS 'Total Order'
                                        FROM sales_fact s
                                        JOIN shipping_method p ON (s.customer_key=p.id)
                                        where name='".$row['name']."'");
    ...
    <!-- Content Wrapper -->
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <!-- Begin Page Content -->
                ...
                <h1 class="h3 mb-2 text-gray-800">Sales - Delivery</h1>
                    <!-- Content Row -->
                    <div class="row">
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">
                                                Total Shipping Methods</div>
                                            <div class="h5 mb-0 font-weight-bold text-gray-800"><?= $item[0]["a"]?></div>
                                        </div>
                                        <div class="col-auto">
                                            <i class="bi bi-truck text-gray-300" style="font-size: 3rem;"></i>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<!-- Script Chart Incomes-->
    <script type="text/javascript">
        ...
        Highcharts.chart('IpC', {
            ...
            title: {
                text: 'Incomes by Shipping Methods'
            },
            accessibility: {
                announceNewData: {
                    enabled: true
