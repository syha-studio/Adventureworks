...
// Summary and Top 10
    ...
    $top10 = query("SELECT SUM(StockedQty) AS 'StockedQty', name, subcategory FROM production_fact f JOIN product p ON (f.product_key=p.id) GROUP BY name
                    ORDER BY StockedQty DESC LIMIT 8");
...
// Chart Order, Stocked, Scrapped and Cost
    ...
    while($row = mysqli_fetch_array($datasub2)){
        ...
        $query = mysqli_query($conn,"SELECT SUM(productionCost) AS 'Total Costs', SUM(OrderQty) AS 'Total Order',
                                            SUM(StockedQty) AS 'Total Stocked', SUM(scrappedQty) AS 'Total Scrapped', category
                                        FROM production_fact s
                                        JOIN product p ON (s.product_key=p.id) 
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <!-- Begin Page Content -->
                <div class="container-fluid">
                    <!-- Content Row -->
                    <h1 class="h3 mb-2 text-gray-800">Production - Product</h1>
                    <div class="row">
                    <!-- Total Product -->
                    ...
                    <!-- Top 10 -->
                        <div class="col-xl-6 col-lg-5">
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Top 8 products with the most production</h6>
                                </div>
                                <div class="d-flex justify-content-center p-2 pb-2">
                                    ...
                                    <tr>
                                        ...
                                        <td><?= number_format($row["StockedQty"], 0, ',', ' ')?></td>
                                    </tr>
                                    <?php $i++; endforeach ?>
                                    </table>
                                </div>
